...
- переменные отправки, которые BAA вычисляет перед POST во внешний endpoint.
...
В этом же меню задаются `Переменные отправки`. Это значения верхнего уровня
запроса, которые не являются атрибутами конкретного создаваемого объекта, но
нужны валидатору как входные параметры. Поддерживаются источники:

- `expression` - выражение BAA, например `${contractparam.environment}`;
- `class` - значение из готовых объектов плана в формате `ClassName.Attribute`
  или `class.ClassName.Attribute`;
- `contractparam` - имя параметра контракта без префикса или
  `contractparam.name`;
- `session` - `username`/`requestId` или `session.username`/`session.requestId`;
- `constant` - значение `DefaultValue`.

Если по `class.ClassName.Attribute` найдено несколько разных значений, BAA
передает массив и помечает переменную как неоднозначную. Для обязательной
переменной это ошибка, и внешний endpoint не вызывается.
